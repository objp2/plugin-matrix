...
const matrixPlugin: Plugin = {
  ...
  init: async (_config: Record<string, string>, runtime: IAgentRuntime) => {
    ...
    logger.debug(
      `Action names: ${(matrixPlugin.actions?.map((a) => a.name) || []).join(", ")}`,
    );
    ...
    setTimeout(async () => {
      try {
        ...
        if (service) {
          ...
          logger.info(
            `Service ready: ${status.isReady}, has client: ${status.hasClient}`,
          );
        } else {
          logger.warn(
            `Matrix service NOT found in runtime after init - service type: ${MatrixService.serviceType}`,
          );
        }
      } catch (error) {
        logger.error(
          "Error checking Matrix service after init:",
          String(error),
        );
      }
    }, 1000);
