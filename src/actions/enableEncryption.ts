  ...
  validate: async (
    ...
    if (!service?.client) {
      if (service && typeof service.getServiceStatus === 'function') {
        logger.debug(`ENABLE_ENCRYPTION unavailable - Matrix service status:`, service.getServiceStatus());
      } else if (service) {
        logger.debug("ENABLE_ENCRYPTION unavailable - Matrix service found but client not ready");
      } else {
        logger.debug("ENABLE_ENCRYPTION unavailable - Matrix service not found");
      }
      return false;
    }
    ...
    if (!content || Object.keys(content).length === 0) {
      logger.debug("ENABLE_ENCRYPTION: No content provided - treating as availability check");
      return true;
    }

    // If content is provided, validate required parameters
    const isValid = !!content.roomId;
    logger.debug(`ENABLE_ENCRYPTION: Content validation result: ${isValid}`, { roomId: !!content.roomId });
    return isValid;
  },
  handler: async (
    runtime: IAgentRuntime,
